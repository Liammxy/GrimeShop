<?php
// Garantir que a sessão esteja iniciada antes de qualquer saída
...
// Calcula a quantidade inicial de itens no carrinho gravados na sessão PHP
$qtd_inicial_carrinho = isset($_SESSION['carrinho']) ? array_sum($_SESSION['carrinho']) : 0;
?>
...
<head>
    ...
    <link rel="icon" type="image/x-icon" href="/ProjetoGrimeShop/images/logoaba.jpeg">
    ...
    <nav class="navbar navbar-expand-lg navbar-dark navbar-cursed p-0" style="border-bottom: 1px solid #ff0033">
        <div class="container">
            ...
            <div class="collapse navbar-collapse d-none d-lg-block" id="navbarNav">
                ...
                <li class="nav-item">
                <a class="nav-link-cursed <?php echo ($pagina_atual == 'carrinho.php') ? 'active' : ''; ?>" href="/ProjetoGrimeShop/paginas/carrinho.php">
                Carrinho <span id="contador-carrinho" class="badge bg-danger ms-1" style="font-size: 0.7rem;"><?php echo $qtd_inicial_carrinho; ?></span>
                </a>
                </li>
                </ul>
            </div>
...
<script>
// Função para sincronizar o número do badge com o total salvo na $_SESSION do PHP
function atualizarContadorCarrinho() {
    const caminhoAdicionar = window.location.pathname.includes('/paginas/') 
        ? 'adicionar_carrinho.php' 
        : 'paginas/adicionar_carrinho.php';

    fetch(caminhoAdicionar)
        ...
        .catch(err => console.error("Erro ao sincronizar contador do carrinho:", err));
}
...
// Captura cliques nos botões "Adicionar ao Carrinho"
document.addEventListener('click', function(e) {
    ...
    if (btn) {
        ...
        // Resolve dinamicamente a rota do script conforme a página atual
        const caminhoAdicionar = window.location.pathname.includes('/paginas/') 
            ? 'adicionar_carrinho.php?id=' + idProduto 
            : 'paginas/adicionar_carrinho.php?id=' + idProduto;

        // Requisição AJAX enviando o produto para a sessão PHP
        fetch(caminhoAdicionar)
            .then(response => response.json())
            .then(data => {
                if (data.status) {
                    // Atualiza o contador do carrinho no menu em tempo real
                    const badge = document.getElementById('contador-carrinho');
                    if (badge) {
                        badge.textContent = data.total_itens;
                    }

                    // Exibe o aviso Toast estilizado
                    ...
                    alert(data.mensagem || 'Erro ao adicionar item.');
                }
            })
            .catch(err => console.error("Erro na requisição do carrinho:", err));
    }
});
