// Bootstrap é carregado via CDN (script global), então só declaramos o
// contrato mínimo que usamos dele — não existe um pacote de tipos instalado.
...
// ---------------------------------------------------------------------------
// Contratos de interface (Modelagem de Dados) - tipam 100% do JSON que vem
// das APIs em PHP. Nenhuma função aqui usa "any".
// ---------------------------------------------------------------------------
...
interface ResumoCategoria {
    id_categoria: number;
    nm_categoria: string;
    qt_produtos_ativos: number;
    vl_total_estoque: number;
    status_mais_critico: string;
}

interface ResumoCategoriaResposta {
    status: boolean;
    dados: ResumoCategoria[];
}

interface RankingProdutoSQL {
    id_produto: number;
    nm_produto: string;
    nm_categoria: string;
    vl_produto: number;
    status_estoque: string;
    qt_total_vendida: number;
    vl_total_vendido: number;
}

interface RankingProdutoResposta {
    status: boolean;
    dados: RankingProdutoSQL[];
}

interface VendasResposta {
    status: boolean;
    total_vendas: number;
}

const STATUS_PEDIDO_VALIDOS: string[] = ['Processando', 'Pago', 'Enviado', 'Entregue', 'Cancelado'];
...
document.addEventListener('DOMContentLoaded', (): void => {
    ...
    if (modalCatEl) {
        modalCatEl.addEventListener('hidden.bs.modal', (): void => {
            const formCat = document.getElementById('form-categoria') as HTMLFormElement | null;
            if (formCat) formCat.reset();

            const idCat = document.getElementById('id_categoria') as HTMLInputElement | null;
            if (idCat) idCat.value = '';

            const labelCat = document.getElementById('modalCategoriaLabel');
            if (labelCat) labelCat.textContent = 'Nova Categoria';
        });
    }

    const modalProdEl = document.getElementById('modalProduto');
    if (modalProdEl) {
        modalProdEl.addEventListener('hidden.bs.modal', (): void => {
            const formProd = document.getElementById('form-produto') as HTMLFormElement | null;
            if (formProd) formProd.reset();

            const idProd = document.getElementById('id_produto') as HTMLInputElement | null;
            if (idProd) idProd.value = '';

            const labelProd = document.getElementById('modalProdutoLabel');
            if (labelProd) labelProd.textContent = 'Novo Produto';
        });
    }

    const formCategoria = document.getElementById('form-categoria') as HTMLFormElement | null;
    if (formCategoria) {
        formCategoria.addEventListener('submit', async (e: Event): Promise<void> => {
            e.preventDefault();
            ...
            try {
                ...
                const res: RespostaApi = await response.json();
...
function obterCaminhoImagem(nomeImagem: string | undefined | null): string | null {
    if (!nomeImagem) return null;
    const apenasNome = nomeImagem.split('/').pop()?.split('\\').pop();
    if (!apenasNome) return null;
...
async function carregarCategorias(): Promise<void> {
    try {
        ...
        const categoriasAtivas: Categoria[] = categorias.filter((c: Categoria) => c.st_categoria === 'ativo');
        const elTotalCat = document.getElementById('total-categorias');
        if (elTotalCat) elTotalCat.textContent = String(categoriasAtivas.length);

        const tbody = document.getElementById('tabela-categorias');
        const selectCategoriaProd = document.getElementById('id_categoria_produto') as HTMLSelectElement | null;

        if (tbody) tbody.innerHTML = '';
        if (selectCategoriaProd) selectCategoriaProd.innerHTML = '<option value="">Selecione uma Categoria...</option>';

        categorias.forEach((cat: Categoria): void => {
...
function abrirEdicaoCategoria(cat: Categoria): void {
    const idCatEl = document.getElementById('id_categoria') as HTMLInputElement | null;
    if (idCatEl) idCatEl.value = String(cat.id_categoria);

    const nmCatEl = document.getElementById('nm_categoria') as HTMLInputElement | null;
    if (nmCatEl) nmCatEl.value = cat.nm_categoria;

    const dsCatEl = document.getElementById('ds_categoria') as HTMLTextAreaElement | null;
    if (dsCatEl) dsCatEl.value = cat.ds_categoria || '';

    const dsBadgeEl = document.getElementById('ds_badge') as HTMLInputElement | null;
    if (dsBadgeEl) dsBadgeEl.value = cat.ds_badge || '';

    const stCatEl = document.getElementById('st_categoria') as HTMLSelectElement | null;
    if (stCatEl) stCatEl.value = cat.st_categoria || 'ativo';

    const lblCatEl = document.getElementById('modalCategoriaLabel');
    if (lblCatEl) lblCatEl.textContent = 'Editar Categoria';
...
async function desativarCategoria(id: number): Promise<void> {
    ...
    try {
        ...
        if (res.status) carregarCategorias();
    } catch (error: unknown) {
        console.error('Erro ao desativar categoria:', error);
    }
}

async function reativarCategoria(id: number): Promise<void> {
    if (!confirm('Deseja reativar esta categoria?')) return;
    try {
...
async function carregarProdutos(): Promise<void> {
    try {
        ...
        const produtosAtivos: Produto[] = produtos.filter((p: Produto) => !p.st_produto || p.st_produto === 'ativo');

        const elTotalProdutos = document.getElementById('total-produtos');
        if (elTotalProdutos) elTotalProdutos.textContent = String(produtosAtivos.length);

        // Uso de Reduce: soma o valor total em estoque (patrimônio)
        const patrimonioTotal: number = produtosAtivos.reduce((acumulador: number, p: Produto): number => {
            const preco = Number(p.vl_preco ?? p.vl_produto ?? 0);
        ...
        const linhasFormatadas: string[] = produtos.map((prod: Produto): string => {
            ...
            const precoValor = Number(prod.vl_preco ?? prod.vl_produto ?? 0);
...
function calcularDestaquesPreco(produtos: Produto[]): void {
    if (!produtos || produtos.length === 0) return;

    // Uso de Map: normaliza o preço de cada produto em um campo numérico único
    const produtosComPreco: ProdutoComPreco[] = produtos.map((p: Produto): ProdutoComPreco => ({
        ...p,
        precoNumerico: Number(p.vl_preco ?? p.vl_produto ?? 0)
    }));

    // Uso de Reduce: encontra o maior e o menor preço da coleção
    const produtoMaisCaro: ProdutoComPreco = produtosComPreco.reduce(
        (max: ProdutoComPreco, atual: ProdutoComPreco) => (atual.precoNumerico > max.precoNumerico ? atual : max),
        produtosComPreco[0]
    );

    const produtoMaisBarato: ProdutoComPreco = produtosComPreco.reduce(
        (min: ProdutoComPreco, atual: ProdutoComPreco) => (atual.precoNumerico < min.precoNumerico ? atual : min),
        produtosComPreco[0]
    );
...
async function carregarRankingMaisVendido(): Promise<void> {
    try {
        ...
        const resItensJson: ItensPedidoResposta = await resItens.json();
        const itensPedido: ItemPedido[] = resItensJson.dados || [];
        ...
        const frequenciaVendas = new Map<number, number>();

        itensPedido.forEach((item: ItemPedido): void => {
            const idProd = item.id_produto;
            const qtd = Number(item.qt_produto) || 0;
            frequenciaVendas.set(idProd, (frequenciaVendas.get(idProd) ?? 0) + qtd);
        });
        ...
        const produtoMaisVendidoObj = produtos.find((p: Produto) => p.id_produto === idMaisVendido);
...
function abrirEdicaoProduto(prod: Produto): void {
    const idProdEl = document.getElementById('id_produto') as HTMLInputElement | null;
    if (idProdEl) idProdEl.value = String(prod.id_produto);

    const nmProdEl = document.getElementById('nm_produto') as HTMLInputElement | null;
    if (nmProdEl) nmProdEl.value = prod.nm_produto;

    const idCatProdEl = document.getElementById('id_categoria_produto') as HTMLSelectElement | null;
    if (idCatProdEl) idCatProdEl.value = String(prod.id_categoria);

    const vlPrecoEl = document.getElementById('vl_preco') as HTMLInputElement | null;
    if (vlPrecoEl) vlPrecoEl.value = String(prod.vl_preco ?? prod.vl_produto ?? 0);

    const qtEstoqueEl = document.getElementById('qt_estoque') as HTMLInputElement | null;
    if (qtEstoqueEl) qtEstoqueEl.value = String(prod.qt_estoque);

    const dsProdEl = document.getElementById('ds_produto') as HTMLTextAreaElement | null;
    if (dsProdEl) dsProdEl.value = prod.ds_produto || '';

    const stProdEl = document.getElementById('st_produto') as HTMLSelectElement | null;
    if (stProdEl) stProdEl.value = prod.st_produto || 'ativo';

    const lblProdEl = document.getElementById('modalProdutoLabel');
    if (lblProdEl) lblProdEl.textContent = 'Editar Produto';
...
async function desativarProduto(id: number): Promise<void> {
    ...
    try {
        ...
        if (res.status) carregarProdutos();
    } catch (error: unknown) {
        console.error('Erro ao desativar produto:', error);
    }
}

async function reativarProduto(id: number): Promise<void> {
...
async function carregarVendas(): Promise<void> {
    try {
        ...
        const res: VendasResposta = await response.json();
...
async function carregarResumoCategoria(): Promise<void> {
    ...
    try {
        ...
        const linhas: string[] = res.dados.map((cat: ResumoCategoria): string => {
...
async function carregarRankingSQL(): Promise<void> {
    ...
    try {
        ...
        const linhas: string[] = res.dados.map(
            (item: RankingProdutoSQL): string => `
                <tr>
...
async function carregarPedidos(): Promise<void> {
    ...
    try {
        ...
        const linhas: string[] = pedidos.map((pedido: Pedido): string => {
            ...
            const opcoesStatus = STATUS_PEDIDO_VALIDOS.map((status: string): string => {
...
async function atualizarStatusPedido(id: number, novoStatus: string): Promise<void> {
    try {
